...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(List())
    case (k, v) :: tailOccurrences => {
      for {
        tailCombinations <- combinations(tailOccurrences)
        i <- 0 to v
      } yield {
        if (i == 0) tailCombinations
        else ((k, i)) :: tailCombinations
      } //sortBy (_._1)
    }
  }
    ...
    def sentenceAnagrams(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case _ =>
        for {
          ...
          otherSentences <- sentenceAnagrams(subtract(occurrences, combinations))
