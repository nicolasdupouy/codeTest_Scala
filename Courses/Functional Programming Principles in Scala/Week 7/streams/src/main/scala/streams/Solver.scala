...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, moves) #:: tail => {
      val newBlock = newNeighborsOnly(neighborsWithHistory(block, moves), explored)

      (block, moves) #:: from(tail ++ newBlock, explored + block)
    }
  }
  ...
  lazy val pathsFromStart: Stream[(Block, List[Move])] =
    from(Stream((this.startBlock, List.empty)), Set.empty)
